...
router.post('/turn_off_on_wa_bot', userValidator, checkIfPlanExpire, async (req, res) => {
    try {
...
router.post('/add_chatbot_wa', userValidator, checkIfPlanExpire, checkGptWordsLimit, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO wa_ai_bot (uid, bot_title, client_id, text_train_data, doc_text_train, train_type, group_id, reaction, enable_typing, openai_model, sql_connection, train_instruction_id, reply_in_groups ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`, [
            req.decode.uid,
            bot_title,
            client_id,
            text_train_data,
